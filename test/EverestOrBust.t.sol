...
contract EverestOrBustTest is Test {
    ...
    // Jan 1 2027 00:00:00 UTC
    uint256 constant START    = 1767225600;
    // Mar 10 2027 00:00:00 UTC (start + 69 days)
    ...
    function test_Contribute_USDC() public {
        ...
        assertEq(campaign.contributedUSDC(alice), 69e6);
        assertEq(campaign.contributedNormalized(alice), 69e18);
        assertEq(campaign.totalRaisedNormalized(), 69e18);
    }

    function test_Contribute_USDT() public {
        vm.startPrank(alice);
        usdt.approve(address(campaign), 69e6);
        campaign.contribute(address(usdt), 69e6);
        vm.stopPrank();

        assertEq(campaign.contributedUSDT(alice), 69e6);
        assertEq(campaign.contributedNormalized(alice), 69e18);
    }

    function test_Contribute_DAI() public {
        vm.startPrank(alice);
        dai.approve(address(campaign), 69e18);
        campaign.contribute(address(dai), 69e18);
        vm.stopPrank();

        assertEq(campaign.contributedDAI(alice), 69e18);
        assertEq(campaign.contributedNormalized(alice), 69e18);
    }
    ...
    function test_Contribute_MixedTokensUpToCap() public {
        ...
        campaign.contribute(address(usdc), 30e6);

        dai.approve(address(campaign), 39e18);
        campaign.contribute(address(dai), 39e18);
        vm.stopPrank();
    ...
    function test_Refund_WhenGoalNotMet() public {
        ...
        assertEq(usdc.balanceOf(alice), balBefore + 69e6);
    ...
    function test_Remaining_DecreasesWithContributions() public {
        ...
        campaign.contribute(address(usdc), 69e6);
        vm.stopPrank();
        assertEq(campaign.remaining(), 69_000e18 - 69e18);
    }

    function test_RemainingCap_DecreasesWithContributions() public {
        assertEq(campaign.remainingCap(alice), 69e18);
        vm.startPrank(alice);
        usdc.approve(address(campaign), 30e6);
        campaign.contribute(address(usdc), 30e6);
        vm.stopPrank();
        assertEq(campaign.remainingCap(alice), 39e18);
    }
    ...
    function _fillGoal() internal {
        uint256 needed = 69_000e18 / 69e18; // 1000 contributors
    ...
    function test_RevertWhen_TokenTransferFailedOnContribute() public {
        ...
        badCampaign.contribute(address(badToken), 69e6);
        vm.stopPrank();
    }
    ...
    function test_RevertWhen_TokenTransferFailedOnRefund() public {
        ...
        vm.startPrank(alice);
        usdc.approve(address(badCampaign), 69e6);
        badCampaign.contribute(address(usdc), 69e6);
        vm.stopPrank();
